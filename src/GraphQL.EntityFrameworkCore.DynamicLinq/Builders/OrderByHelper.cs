...
internal class OrderByHelper<T>
{
    ...
    public OrderByHelper(QueryArgumentInfoList list, IDictionary<string, ArgumentValue> arguments)
    {
        _list = list;
    ...
    {
        if (!_arguments.TryGetValue(FieldNames.OrderByFieldName, out var value))
        {
            return null;
        }

        var orderByStatement = Convert.ToString(value.Value);
